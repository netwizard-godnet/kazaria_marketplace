...
class AuthController extends Controller
{
    ...
    public function verifyLoginCode(Request $request)
    {
        ...
        $isApiRoute = $request->is('api/*') 
            ...
            || ($request->route() && strpos($request->route()->uri(), 'api/') === 0);

        // Pour les routes API, toujours utiliser des tokens Sanctum (pas de session)
        if ($isApiRoute) {
            try {
        ...
        // Route web : utiliser les sessions (essayer sans vérifier d'abord)
        try {
            // Essayer d'utiliser les sessions - si ça échoue, on utilisera un token
            // Ne pas vérifier hasSession() car cela peut lancer une exception
