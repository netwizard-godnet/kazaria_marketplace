        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="slug" class="form-control" value="{{ old('slug', $popup->slug) }}" placeholder="Laissez vide pour générer automatiquement">
                    <small class="text-muted">Utilisé comme identifiant unique pour la popup.</small>
                </div>

                <div class="mb-3">
                    <label class="form-label">Contenu HTML</label>
                    <textarea name="content" class="form-control" rows="8" placeholder="HTML de votre popup">{{ old('content', $popup->content) }}</textarea>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="cta_text" class="form-control" value="{{ old('cta_text', $popup->cta_text) }}" placeholder="Ex: Découvrir l'offre">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Lien du bouton</label>
                        <input type="url" name="cta_url" class="form-control" value="{{ old('cta_url', $popup->cta_url) }}" placeholder="https://">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="display_start" class="form-control"
                               value="{{ old('display_start', optional($popup->display_start)->format('Y-m-d\TH:i')) }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" name="display_end" class="form-control"
                               value="{{ old('display_end', optional($popup->display_end)->format('Y-m-d\TH:i')) }}">
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-2">
                        @foreach($pagePresets as $key => $label)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="page_{{ $key }}" name="display_pages[]" value="{{ $key }}"
                                       {{ in_array($key, $presetSelection ?? [], true) ? 'checked' : '' }}>
                                <label class="form-check-label" for="page_{{ $key }}">{{ $label }}</label>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="form-check form-switch">
                        @foreach($devices as $key => $label)
                            <div class="form-check form-switch form-check-inline">
                                <input class="form-check-input" type="checkbox" id="device_{{ $key }}" name="display_devices[]" value="{{ $key }}"
                                       {{ in_array($key, $selectedDevices ?? [], true) ? 'checked' : '' }}>
                                <label class="form-check-label" for="device_{{ $key }}">{{ $label }}</label>
                            </div>
                        @endforeach
                    </div>
                </div>
    ...
    <div class="col-lg-4">
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="frequency" class="form-select">
                        @foreach($frequencies as $value => $label)
                            <option value="{{ $value }}" {{ old('frequency', $popup->frequency ?? 'once_per_session') === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="layout" class="form-select">
                        @foreach($layouts as $value => $label)
                            <option value="{{ $value }}" {{ old('layout', $popup->layout ?? 'left-right') === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    @if($popup->image)
                        <div class="mb-2">
                            <img src="{{ Str::startsWith($popup->image, ['http://', 'https://']) ? $popup->image : asset('storage/' . ltrim($popup->image, '/')) }}" alt="Image popup" class="img-fluid rounded border">
                        </div>
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" name="remove_image" value="1" id="remove_image">
                            <label class="form-check-label" for="remove_image">Supprimer l'image actuelle</label>
                        </div>
                        <input type="hidden" name="image_path" value="{{ $popup->image }}">
                    @endif
        ...
        <div class="card">
            <div class="card-body">
                ...
                <a href="{{ route('admin.popups.index') }}" class="btn btn-outline-secondary w-100 mt-2">
                    Annuler
                </a>
            </div>
        </div>
